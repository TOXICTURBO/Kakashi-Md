let handler = async (m, { conn, command, args, text, usedPrefix }) => {
  try {
    ...
    let _lmao = args[0]
    let Lmao = `only support 1, 10, 100, 1000\nExamples of use: *${usedPrefix}open ${args > 2 ? _lmao : pickRandom(['common', 'uncommon', 'mythic', 'legendary'])} 10*`
...
`.trim()
                    ...
                    } else conn.reply(m.chat, '📦Common crate you are not enough', m)
                    break
                case '10':
...
`.trim()
                    if (global.db.data.users[m.sender].uncommon >= 1) {
                        ...
                        if (umc > 0) {
                            m.reply(`*Congratulations, you got a rare item that is*\n${umc} Mythic Crate`)
                            global.db.data.users[m.sender].mythic += umc * 1
                        }
                    } else conn.reply(m.chat, '📦Uncommon crate you are not enough', m)
                    break
                case '10':
...
`.trim()
                    if (global.db.data.users[m.sender].mythic >= 1) {
                        ...
                        if (ml > 0) {
                            m.reply(`*Congratulations you got the Epic item that is*\n${ml} Legendary Crate`)
                            global.db.data.users[m.sender].legendary += ml * 1
                        }
                    } else conn.reply(m.chat, '📦Mythic crate you are not enough', m)
                    break
                case '10':
...
`.trim()  
                    if (global.db.data.users[m.sender].legendary >= 1) {
                        ...
                        if (ll > 0 || lpp > 0) {
                             m.reply(`*Congratulations you got the Epic item that is*${ll > 0 ? `\n${ll} Legendary Crate` : ''}${lpp > 0 ? `\n${lpp} Pet Crate` : ''}`)
                            global.db.data.users[m.sender].legendary += ll * 1
                            global.db.data.users[m.sender].pet += lpp * 1
                        }
                    } else conn.reply(m.chat, 'Legendary crate you are not enough', m)
                    break
                case '10':
...
`.trim()  
                    if (global.db.data.users[m.sender].legendary >= 10) {
                        ...
                        if (ll1 > 0 || lpp1 > 0) {
                             m.reply(`*Congratulations you got the Epic item that is*${ll1 > 0 ? `\n${ll1} Legendary Crate` : ''}${lpp1 > 0 ? `\n${lpp1} Pet Crate` : ''}`)
...
`.trim()  
                    if (global.db.data.users[m.sender].legendary >= 100) {
                        ...
                        if (ll2 > 0 || lpp2 > 0) {
                             m.reply(`*Congratulations you got the Epic item that is*${ll2 > 0 ? `\n${ll2} Legendary Crate` : ''}${lpp2 > 0 ? `\n${lpp2} Pet Crate` : ''}`)
...
`.trim()  
                    if (global.db.data.users[m.sender].legendary >= 1000) {
                        ...
                        if (ll3 > 0 || lpp3 > 0) {
                             m.reply(`*Congratulations you got the Epic item that is*${ll3 > 0 ? `\n${ll3} Legendary Crate` : ''}${lpp3 > 0 ? `\n${lpp3} Pet Crate` : ''}`)
        ...
        case 'pet':
            ...
            if (global.db.data.users[m.sender].pet > 0) { 
                ...
                if (_pet == 'kucing' && kucing > 0) {
                    ...
                    conn.reply(m.chat, `You already have a pet ${_pet}, Hadiahmu diganti dengan 2 potion${mknp > 0 ? ` And ${mknp} Pet Food` : ''}`, m)
                } else if (_pet == 'kucing' && kucing == 0) {
                    global.db.data.users[m.sender].kucing += 1
                    global.db.data.users[m.sender].makananpet += mknp * 1
                    conn.reply(m.chat, `*Congratulations you got a pet${_pet} ${mknp > 0 ? ` And ${mknp} Pet Food*` : '*'}`, m)
                    ...
                    global.db.data.users[m.sender].makananpet += mknp * 1
                    m.reply(pickRandom(['You are out of luck', 'Try opening it again next time, because I didnt get a pet', 'poor I didnt get a pet', 'Maybe its not hockey and I dont get a pet', 'wkwkkwkwke']) + '. you only get *' + mknp + '* Pet Food')
                }
